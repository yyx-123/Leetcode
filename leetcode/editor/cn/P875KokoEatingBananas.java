...
public class P875KokoEatingBananas{
    ...
    class Solution {
        public int minEatingSpeed(int[] piles, int h) {
            long left = 1, right = 0;
            for (int p : piles) right += p;

            while (left <= right){
                long mid = left + (right - left) / 2;

                // 这个吃香蕉速度可以，那么可以再减慢一点速度，移动right
                if (check(mid, h, piles)){
                    right = mid - 1;
                }
                // 这个吃香蕉速度太慢了，需要加快速度，移动left
                else{
                    left = mid + 1;
                }
            }
        ...
        // 检查以速度v吃香蕉的话能不能再h小时内吃完，能的话返回true
        private boolean check(long v, int h, int[] piles) {
            long cnt = 0;
            for (int p : piles){
                cnt += p / v;
                if (p % v != 0) cnt++;
