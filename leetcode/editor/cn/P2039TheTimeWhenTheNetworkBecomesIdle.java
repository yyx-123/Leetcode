...
//2022-03-20 10:05:14
public class P2039TheTimeWhenTheNetworkBecomesIdle{
    public static void main(String[] args) {
        Solution solution = new P2039TheTimeWhenTheNetworkBecomesIdle().new Solution();
        // TO TEST
    ...
    class Solution {
        public int networkBecomesIdle(int[][] edges, int[] patience) {
            ...
            buildRelations(edges, relations);

            int max = -1;
            for (int i = 1; i < relations.size(); i++) {
                int dist = calDist(i, relations);

                if (2 * dist <= patience[i]){
                    max = Math.max(max, 2 * dist);
                }else{
                    max = Math.max(max, (2 * dist - 1) / patience[i] * patience[i] + 2 * dist);
                }
            }
            return max + 1;
        }

        private int calDist(int i, Map<Integer, List<Integer>> relations) {
        ...
        private void buildRelations(int[][] edges, Map<Integer, List<Integer>> relations) {
            for (int[] edge : edges) {
                int u = edge[0], v = edge[1];

                List<Integer> list = relations.getOrDefault(u, new ArrayList<>());
                list.add(v);
                relations.put(u, list);

                list = relations.getOrDefault(v, new ArrayList<>());
                list.add(u);
                relations.put(v, list);
            }
        }
    }
