    ...
    class Solution {
        ...
        public String shortestCompletingWord(String licensePlate, String[] words) {
            for (char c : licensePlate.toLowerCase().toCharArray()){
                if (Character.isLetter(c)) charCnt[c - 'a']++;
            }

            int rstIdx = -1, minLen = Integer.MAX_VALUE;
        ...
        private boolean isValid(String word) {
            int[] wordCharCnt = new int[26];
            for (char c : word.toCharArray()) wordCharCnt[c - 'a']++;
            for (int i = 0; i < 26; i++) {
                if (charCnt[i] > wordCharCnt[i]) return false;
            }
            return true;
        }
    }
