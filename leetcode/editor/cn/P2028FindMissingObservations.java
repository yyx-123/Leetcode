...
//2022-03-27 10:02:31
public class P2028FindMissingObservations{
    public static void main(String[] args) {
        Solution solution = new P2028FindMissingObservations().new Solution();
        // TO TEST
    ...
    class Solution {
        public int[] missingRolls(int[] rolls, int mean, int n) {
            int m = rolls.length, curSum = sum(rolls), tgtSum = (m + n) * mean;
            int diff = tgtSum - curSum;

            if (diff < n || diff > n * 6) return new int[]{};

            int[] rst = new int[n];
            Arrays.fill(rst, diff / n);
            for (int i = 0; i < diff - diff / n * n; i++) {
                rst[i]++;
            }
            return rst;
        }
